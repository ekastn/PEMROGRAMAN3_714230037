...
export const getMahasiswaByNpm = async (npm) => {
    const response = await axios.get(`${API_BASE_URL}/${npm}`);
    return response.data.data;
};

// PUT: Update Mahasiswa
export const updateMahasiswa = async (npm, payload) => {
    const response = await axios.put(`${API_BASE_URL}/${npm}`, payload);
    return response.data;
};

// DELETE Mahasiswa
export const deleteMahasiswa = async (npm) => {
  const response = await axios.delete(`${API_BASE_URL}/${npm}`);
  return response.data;
};
